...
#
# `container_name` is fixed to "staging" because the bootstrap script
# creates one container per environment (staging, production) inside
# the same storage account — keeping per-env containers isolates
# blob-level RBAC.
#
...
terraform {
  backend "azurerm" {
    # Keep in lockstep with environments/production/backend.tf — only
