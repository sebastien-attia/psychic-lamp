...
# Idempotence
# ───────────
# The SQL is wrapped in DO blocks that check for existing roles before
# CREATE; ALTER ROLE ... PASSWORD is unconditional so password rotation
# propagates without manual intervention. GRANTs are idempotent (re-running
# is safe).
#
      ...
      $PSQL --dbname=postgres --variable=pw="$BFF_DB_PASSWORD" --command="
        ...
        BEGIN
          IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'bff') THEN
            CREATE ROLE bff LOGIN PASSWORD :'pw';
          ELSE
      ...
      $PSQL --dbname=postgres --variable=pw="$BUSINESS_DB_PASSWORD" --command="
        ...
        BEGIN
          IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'business_service') THEN
            CREATE ROLE business_service LOGIN PASSWORD :'pw';
          ELSE
